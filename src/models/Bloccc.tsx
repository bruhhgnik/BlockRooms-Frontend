...
const TRIGGER_DIST   = 10.0;      // turn on when player is within 10m
const AUTO_OFF_MS    = 4000;      // 2s grace period
...
function FluoroFixture({ nodeName }: { nodeName: string }) {
  ...
  useFrame(() => {
    ...
    const holes = (window as any).__LIGHT_HOLES;

    lightPositionsLocal.forEach((lp, i) => {
      tmpLocal.current.set(lp[0], lp[1], lp[2]);
      tmpWorld.current.copy(tmpLocal.current);
      group.current.localToWorld(tmpWorld.current);

      const light = lightRefs.current[i];
      if (!light) return;

      const dist = tmpWorld.current.distanceTo(camPos.current);
      const now = performance.now();

      if (dist <= TRIGGER_DIST) {
        light.visible = true;
        light.intensity = LIGHT_INTENSITY;
        lastOnAt.current[i] = now;

        if (Array.isArray(holes)) {
          holes.push({ x: tmpWorld.current.x, y: tmpWorld.current.y, z: tmpWorld.current.z, r: 4.2 });
        }
      } else {
        if (now - lastOnAt.current[i] > AUTO_OFF_MS) {
          light.visible = false;
        } else {
          light.visible = true;
          if (Array.isArray(holes)) {
...
<CorridorLights />
      ...
      <mesh
        ...
        material={materials.Ceiling_Light_Surrounds}
      ...
      <mesh
        ...
        position={[-34.777, 1.048, -0.067]}
        rotation={[0, Math.PI / 2, 0]}
        scale={[0.151, 1.464, 4.557]}
      />
