...
async function getCounterValue(
  ...
  counterPDA: anchor.web3.PublicKey,
...
async function assertCounterValue(
  program: Program<Contract>,
  counterPDA: anchor.web3.PublicKey,
  connection: anchor.web3.Connection,
  ...
  const actualValue = await getCounterValue(program, counterPDA, connection);
  expect(actualValue).to.equal(expectedValue, message);
}

describe("need-for-bits counter", () => {
